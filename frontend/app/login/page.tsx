...
import { loginUser, forgotPassword, googleAuth } from "../../services/auth";
  ...
  useEffect(() => {
    ...
    const fetchGoogleConfig = async () => {
      try {
        ...
        // Keep UI fallback when runtime config cannot be fetched.
      }
    };
              ...
              <div className="animate-fade-up delay-700">
                <div className="flex justify-center min-h-[44px] mb-3">
                  {resolvedGoogleClientId ? (
                    ...
                    <p className="text-xs text-white/35">Google sign in is unavailable. Configure GOOGLE_CLIENT_ID in backend or NEXT_PUBLIC_GOOGLE_CLIENT_ID in frontend.</p>
                  )}
                </div>
                {isGoogleLoading && (
                  <p className="text-center text-xs text-white/35 mb-3">Signing you in with Google...</p>
                )}
                <button className="btn-secondary w-full flex items-center justify-center gap-2.5 py-2.5 rounded-xl text-sm font-medium text-white/70 transition-all" disabled>
